...
3. 駅の栄え度グレードは `src/lib/stationAreaTiers.ts` で station code をキーに管理する。主指標は国土数値情報の駅別乗降客数とし、MVPでは手動seedから始める。詳細な基準は [STATION_AREA_TIERS.md](STATION_AREA_TIERS.md) を参照する
4. 栄え度は直接加点・減点しない。スコア差が3分以内の候補群だけ、高グレードの駅を先に並べる。未登録駅は内部tier 0として補正しない
5. スコア昇順に走査し、採択済み候補との「距離が0.6km未満」または「`original_name` が同一かつ距離1.2km以下」のものはスキップ（実質同じ場所の重複排除）。`original_name` が同一でも距離が離れていれば別の場所なので捨てない
6. 上位5件を返す。各候補に付随する情報: 駅情報、栄え度グレード、各メンバーの所要時間（分、四捨五入で整数表示用の値も）、max、mean
7. 候補が0件（鉄道網が分断されている入力）の場合、どのメンバーが他と分断されているかを返す:
...
5. 結果リスト: 順位付きカード×最大5
   ...
   - 手動グレード登録済みの駅だけ、駅名の横に「栄え度 {S/A/B/C/D}」を表示する
...
現行版は、静的ホスティングできることを優先して外部APIを使わない。終電判定、Google Maps API連携、駅の栄え度グレードを入れる場合は、MVPとは別フェーズとして設計する。
...
| 駅の栄え度グレード | 駅コードをキーにした静的データから始める。スコアリングでは、移動時間の公平性を主軸にしつつ、同点または近い候補の並び替えに使う。表示はS/A/B/C/Dとする。詳細は [STATION_AREA_TIERS.md](STATION_AREA_TIERS.md) |

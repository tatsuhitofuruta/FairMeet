...
3. 駅の栄え度tierは `src/lib/stationAreaTiers.ts` で station code をキーに管理する。主指標は国土数値情報の駅別乗降客数とし、MVPでは手動seedから始める。詳細な基準は [STATION_AREA_TIERS.md](STATION_AREA_TIERS.md) を参照する
4. tierは直接加点・減点しない。スコア差が3分以内の候補群だけ、高tierの駅を先に並べる。未登録駅はtier 0として補正しない
...
6. 上位5件を返す。各候補に付随する情報: 駅情報、栄え度tier、各メンバーの所要時間（分、四捨五入で整数表示用の値も）、max、mean
...
現行版は、静的ホスティングできることを優先して外部APIを使わない。終電判定、Google Maps API連携、駅の栄え度tierを入れる場合は、MVPとは別フェーズとして設計する。
